...
- Compact large y-axis tick labels so charts remain readable, such as `250,000` → `250k`, `1,200,000` → `1.2M`, and `3,400,000,000` → `3.4B`.
...
- Default title pattern: `<metric> by <dimension/grouping>` or `<metric> over time by <grouping>`.
  ...
  - Avoid by default: `Sonnet 4.6 and Opus 4.7 drive revenue`
...
```text
artifacts/data-agent/<descriptive-slug>/
```
...
Use this bounded query as a basic plotting smoke test when ClickHouse MCP access is available. It should produce a multi-line or small-multiple time-series chart with `report_date` on x, `tasks_created` on y, and `platform` as the color/group dimension. Save both CSV and a plain HTML/CSS/JavaScript/SVG interactive artifact.

```sql
WITH top_platforms AS (
  SELECT platform
  FROM analytics_prd.gold_task_created_daily
  WHERE report_date >= today() - 30
    AND report_date < today()
    AND platform IS NOT NULL
    AND platform != ''
  GROUP BY platform
  ORDER BY sum(task_created_count) DESC
  LIMIT 5
)
SELECT
  report_date,
  platform,
  sum(task_created_count) AS tasks_created,
  uniqExact(user_id) AS active_users,
  round(tasks_created / nullIf(active_users, 0), 2) AS tasks_per_active_user
FROM analytics_prd.gold_task_created_daily
WHERE report_date >= today() - 30
  AND report_date < today()
  AND platform IN (SELECT platform FROM top_platforms)
GROUP BY report_date, platform
ORDER BY report_date, tasks_created DESC
```

Expected plot contract for this query:

- chart type: start with multi-line chart only if the top platforms are comparable in magnitude; otherwise prefer small multiples or ask whether the user wants absolute volume or trend-shape comparison
- x: `report_date`
- y: `tasks_created`
- color/group: `platform`
- source: `analytics_prd.gold_task_created_daily`
- caveats: last 30 complete days; top 5 platforms by total task-created count
